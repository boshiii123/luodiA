...
firebase.initializeApp({
    "messagingSenderId": "353793449981"
});
...
scriptLoaded.then(function() {
    ...
    function subscribe() {
        ...
        if (isWrongBrowser()) {
            ...
            navigator.serviceWorker.register(window.location.origin + swScope + customWorkerJS, {
                scope: swScope
    ...
    function connectDB(f) {
        ...
        request.onupgradeneeded = function(event) {
            ...
            if (!thisDB.objectStoreNames.contains(indexedDBConfig.baseName)) {
                var store = event.currentTarget.result.createObjectStore(indexedDBConfig.storeName, {
                    autoIncrement: true
                });
